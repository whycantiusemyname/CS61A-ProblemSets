...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while True:
            if current_place.bees and self.lower_bound <= search_steps <= self.upper_bound:
                return random_bee(current_place.bees)
            else:
                if not current_place.entrance.is_hive:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            fire_damage = amount + self.damage
        else:
            fire_damage = amount
        for bee in self.place.bees[:]:
            bee.reduce_health(fire_damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if not self.turns_to_chew:
            chew_bee = random_bee(self.place.bees)
            if chew_bee:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while buff_place:
            buffee = buff_place.ant
            if buffee:
                if buffee.is_container and buffee.ant_contained:
                    buffee.ant_contained.double()
                buffee.double()
